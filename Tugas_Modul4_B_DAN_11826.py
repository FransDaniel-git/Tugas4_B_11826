...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    ...
    if model_name == "KMeans_model":
        ...
        #pada agglomerativ dan dbscan dilakukan dengan cara menghitung jarak titik terdekat
        distances = pairwise_distances(new_point[features], data[features])
        nearest_index = distances.argmin()
        new_cluster = clusters[nearest_index]  # Convert to string
...
st.set_page_config(
    page_title="11826 - Unsupervised Learning", #XXXXX diisi dengan 5 digit NPM
    page_icon="🤖",
    layout="wide",
...
if uploaded_file is not None:
    ...
    st.markdown(f"<h1 style='text-align: center;'>Unsupervised Learning - Frans</h1>", unsafe_allow_html=True) #YYYYY diisi dengan nama panggilan
